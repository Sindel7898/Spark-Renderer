...
if (WIN32)
    if(${COMPILER_REAL_PATH} MATCHES "arm64")
        find_path(NvPerf_OS_INCLUDE_DIR "nvperf_host_impl.h" HINTS ${NvPerf_search_paths} PATH_SUFFIXES "include/windows-desktop-a64")
        find_library(NvPerfHost_LIBRARY NAMES nvperf_grfx_host   HINTS ${NvPerf_search_paths} PATH_SUFFIXES "bin/a64")
    else()
        find_path(NvPerf_OS_INCLUDE_DIR "nvperf_host_impl.h" HINTS ${NvPerf_search_paths} PATH_SUFFIXES "include/windows-desktop-x64")
        find_library(NvPerfHost_LIBRARY NAMES nvperf_grfx_host   HINTS ${NvPerf_search_paths} PATH_SUFFIXES "bin/x64")
    endif()
elseif(QNX)
    find_path(NvPerf_OS_INCLUDE_DIR "nvperf_host_impl.h" HINTS ${NvPerf_search_paths} PATH_SUFFIXES "include/qnx-generic-a64")
    find_library(NvPerfHost_LIBRARY NAMES nvperf_grfx_host   HINTS ${NvPerf_search_paths} PATH_SUFFIXES "lib/a64")
else()
    if(${COMPILER_REAL_PATH} MATCHES "aarch64")
        ...
        find_library(NvPerfHost_LIBRARY NAMES nvperf_grfx_host   HINTS ${NvPerf_search_paths} PATH_SUFFIXES "lib/x64")
    endif()
endif()
...
if (NvPerf_FOUND)
    ...
    function(DeployNvPerf deployTarget nvPerfTarget)
        ...
        if (WIN32)
            ...
            add_custom_command(TARGET ${deployTarget} POST_BUILD
                COMMAND cp -P -f ${NvPerf_DEPLOY_LIBRARY}* $<TARGET_FILE_DIR:${deployTarget}>
            )
        endif()
    endfunction()
else()
