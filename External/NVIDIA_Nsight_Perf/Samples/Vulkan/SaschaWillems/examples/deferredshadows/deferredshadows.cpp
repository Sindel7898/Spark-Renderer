...
public:
	...
	virtual VkResult createInstance(bool enableValidation)
	{
		...
		nv::perf::VulkanAppendInstanceRequiredExtensions(enabledInstanceExtensions, apiVersion);
		return VulkanExampleBase::createInstance(enableValidation);
	}
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		nv::perf::VulkanAppendDeviceRequiredExtensions(instance, physicalDevice, (void*)vkGetInstanceProcAddr, enabledDeviceExtensions);
#endif
	}
	...
	{
		offScreenCmdBuffers.resize(swapChain.imageCount);
		for (size_t swapchainIndex = 0; swapchainIndex < swapChain.imageCount; ++swapchainIndex)
		{
			VkCommandBuffer& offScreenCmdBuffer = offScreenCmdBuffers[swapchainIndex];
	...
	void buildDeferredCommandBuffers()
	{
		for (size_t swapchainIndex = 0; swapchainIndex < swapChain.imageCount; ++swapchainIndex)
		{
			m_apiTracers[swapchainIndex].ClearData();
			VkCommandBuffer& offScreenCmdBuffer = offScreenCmdBuffers[swapchainIndex];
			...
			auto& apiTracer = m_apiTracers[swapchainIndex];
			apiTracer.ResetQueries(offScreenCmdBuffer);
			if (!apiTracer.BeginRange(offScreenCmdBuffer, "Frame", 0, m_frameLevelTraceIndice[swapchainIndex]))
			{
				NV_PERF_LOG_ERR(10, "Failed to BeginRange \"Frame\" \n");
			}

			size_t index = 0;
			if (!apiTracer.BeginRange(offScreenCmdBuffer, "ShadowMap", 1, index))
			{
				NV_PERF_LOG_ERR(10, "Failed to BeginRange \"ShadowMap\"\n");
			}
			...
			if (!apiTracer.EndRange(offScreenCmdBuffer, index))
			{
				NV_PERF_LOG_ERR(10, "Failed to EndRange\n");
			}
			...
			if (!apiTracer.BeginRange(offScreenCmdBuffer, "Deferred calculations", 1, index))
			{
				NV_PERF_LOG_ERR(10, "Failed to BeginRange \"Deferred calculations\"\n");
			}
			vkCmdBeginRenderPass(offScreenCmdBuffer, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
	...
	{
#if NV_PERF_ENABLE_INSTRUMENTATION
		// Rebuild the API trace data inside
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
			auto& apiTracer = m_apiTracers[i];
			size_t index = 0;
			if (!apiTracer.BeginRange(drawCmdBuffers[i], "draw", 1, index))
			{
				NV_PERF_LOG_ERR(10, "Failed to BeginRange \"draw\"\n");
			}
#endif
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
			if (!apiTracer.EndRange(drawCmdBuffers[i], index))
			{
				NV_PERF_LOG_ERR(10, "Failed to EndRange\n");
			}

			if (!apiTracer.BeginRange(drawCmdBuffers[i], "drawUI", 1, index))
			{
				NV_PERF_LOG_ERR(10, "Failed to BeginRange \"drawUI\"\n");
			}
#endif
			...
			if (!apiTracer.EndRange(drawCmdBuffers[i], m_frameLevelTraceIndice[i]))
			{
				NV_PERF_LOG_ERR(10, "Failed to EndRange\n");
			}
#endif
	...
	void prepare()
	{
		...
		const size_t numFramesToSample = swapChain.imageCount;
		// Ranges:
		// 0: Frame
		// 1: ShadowMap
		...
		ThrowIfFalse(nv::perf::InitializeNvPerf(), "InitializeNvPerf failed");
		ThrowIfFalse(nv::perf::VulkanLoadDriver(instance), "Could not load driver");
		nv::perf::DeviceIdentifiers deviceIdentifiers = nv::perf::VulkanGetDeviceIdentifiers(instance, physicalDevice, device);
		ThrowIfFalse(deviceIdentifiers.pChipName, "Could not get device identifiers.");

		// Load metric list to configure
		...
		ThrowIfFalse(hudDataModel.Load(hudPresets.GetPreset(hudConfigurationName)), "Failed to get hud preset.");

		// Load metric configuration
		std::string metricConfigName;
		ThrowIfFalse(nv::perf::MetricConfigurations::GetMetricConfigNameBasedOnHudConfigurationName(metricConfigName, deviceIdentifiers.pChipName, hudConfigurationName), "Failed to find metric configuration.");
		nv::perf::MetricConfigObject metricConfigObject;
		ThrowIfFalse(nv::perf::MetricConfigurations::LoadMetricConfigObject(metricConfigObject, deviceIdentifiers.pChipName, metricConfigName), "Failed to load metric configuration.");

		ThrowIfFalse(hudDataModel.Initialize(metricConfigObject), "Failed to initialize hud data model.");

		ThrowIfFalse(m_periodicSamplerOneShot.Initialize(instance, physicalDevice, device, samplingIntervalInNanoSeconds, maxIntervalPerFrameInNanoSeconds, numFramesToSample, onStopSampling, hudDataModel.GetCounterConfiguration()), "Failed to initialize the one-shot periodic sampler.");
#ifdef WIN32
		...
		{
			ThrowIfFalse(apiTracer.Initialize(instance, physicalDevice, device, numRangesPerFrame), "Failed to initialize the Vulkan API tracer.");
		}
#endif
		loadAssets();
		deferredSetup();
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		submitInfo.pCommandBuffers = &offScreenCmdBuffers[currentBuffer];
#else
	...
	virtual void render()
	{
		...
		if (!m_outputDirectory.empty())
		{
			...
			const std::string filename = nv::perf::utilities::JoinDriectoryAndFileName(m_outputDirectory, "traces.yaml");
		...
		static const size_t startNvPerfTitlePos = title.length();
		PeriodicSamplerOneShotVulkan::SamplerStatus samplerStatus = m_periodicSamplerOneShot.GetSamplerStatus();
		if (samplerStatus == PeriodicSamplerOneShotVulkan::SamplerStatus::Failed)
		{
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Failed to initialize");
		}
		else if (samplerStatus == PeriodicSamplerOneShotVulkan::SamplerStatus::Initialized)
		{
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Hit <space-bar> to start sampling");
		}
		else if (samplerStatus == PeriodicSamplerOneShotVulkan::SamplerStatus::Sampling)
		{
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Currently sampling, counter data will be writen to: <CWD>\\OneShot\\deferredshadows\\<YYYYMMDD_HHMMSS>");
		}
