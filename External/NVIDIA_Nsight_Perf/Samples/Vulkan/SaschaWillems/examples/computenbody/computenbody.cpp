...
public:
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		m_nvperf.rangeCommands.PushRange(compute.commandBuffer, "Dispatch");
#endif
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		m_nvperf.rangeCommands.PopRange(compute.commandBuffer);
#endif
	...
	void prepare()
	{
		...
		m_nvperf.SetMaxNumRanges(2); // "Frame" + "Dispatch"
#ifdef WIN32
  		m_nvperf.outputOptions.directoryName = "HtmlReports\\computenbody";
#else
  		m_nvperf.outputOptions.directoryName = "./HtmlReports/computenbody";
#endif
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		m_nvperf.OnFrameStart(compute.queue, compute.queueFamilyIndex);
#endif
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		// the last OnFrameEnd() will evaluate the collected metrics which causes a frame spike.
		//  updateComputeUniformsBuffers() above uses the previous frames time to update the buffers, so
		//  don't unpause the simulation until after updating the compute uniform buffers on the frame
		//  after collection is finished so the frame spike doesn't affect the simulation itself
		static bool wasNvPerfCollecting = false;
		bool isNvPerfCollecting = m_nvperf.IsCollectingReport();
		if (wasNvPerfCollecting != isNvPerfCollecting)
		{
			wasNvPerfCollecting = isNvPerfCollecting;
			if (!isNvPerfCollecting)
			{
				paused = false;
			}
		}
	...
	virtual void keyPressed(uint32_t key)
	{
		...
		{
		case KEY_SPACE:
			if (!m_nvperfWarmupTime)
			{
				m_nvperf.StartCollectionOnNextFrame();
				// force pause of the simulation during collection
				paused = true;
				break;
			}
		}
	...
	{
		m_nvperfWarmupTime = frameTimer > m_nvperfWarmupTime ? 0.0 : m_nvperfWarmupTime - frameTimer;
		ImGui::TextUnformatted("Nsight Perf:");
		if (m_nvperf.IsCollectingReport())
		{
			ImGui::TextUnformatted("  Currently profiling the frame.");
			ImGui::TextUnformatted("  HTML Report will be written to:");
#ifdef WIN32
			ImGui::TextUnformatted("    <CWD>\\HtmlReports\\computenbody");
#else
			ImGui::TextUnformatted("    ./HtmlReports/computenbody");
#endif
		}
		else if (m_nvperfWarmupTime)
		{
			// Wait for the frame to stabilize, including GPU clocks, memory uploads, caches, and shader compilation.
			ImGui::TextUnformatted("  Waiting for device to warm-up.");
		}
		else if (m_nvperf.GetInitStatus() == nv::perf::profiler::ReportGeneratorInitStatus::Succeeded)
		{
			ImGui::TextUnformatted("  Hit <space-bar> to begin profiling.");
		}
		else
		{
			ImGui::TextUnformatted("  Initialization failed. Please check the logs.");
		}
	}
#endif
};
