...
public:
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		nv::perf::sampler::PeriodicSamplerTimeHistoryVulkan::AppendDeviceRequiredExtensions(apiVersion, enabledDeviceExtensions);
#endif
	};
	...
	void initHUD()
	{
		sampler.Initialize(instance, physicalDevice, device);
		const nv::perf::DeviceIdentifiers deviceIdentifiers = sampler.GetGpuDeviceIdentifiers();
		const uint32_t                    maxFrameLatency   = swapChain.imageCount + 1;
		sampler.BeginSession(queue, vulkanDevice->queueFamilyIndices.graphics, samplingIntervalInNanoSeconds, maxDecodeLatencyInNanoSeconds, maxFrameLatency);
		...
		hudDataModel.Initialize(1.0 / (double) samplingFrequency, plotTimeWidthInSeconds, metricConfigObject);
