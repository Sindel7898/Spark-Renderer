...
public:
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
			m_nvperf.rangeCommands.PushRange(drawCmdBuffers[i], "Draw");
#endif
			vkCmdDrawIndexed(drawCmdBuffers[i], indices.count, 1, 0, 0, 1);
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
			m_nvperf.rangeCommands.PopRange(drawCmdBuffers[i]);
#endif
...
#ifdef WIN32
		m_nvperf.outputOptions.directoryName = "HtmlReports\\triangle";
#else
		m_nvperf.outputOptions.directoryName = "./HtmlReports/triangle";
#endif
	...
	virtual void render()
	{
...
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
		m_nvperf.OnFrameStart(queue, vulkanDevice->queueFamilyIndices.graphics);
#endif
		...
		{
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Currently profiling the frame. HTML Report will be written to: <CWD>\\PerfWorks\\Vulkan\\triangle");
		}
		else if (m_nvperfWarmupTime)
		{
			...
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Waiting for device to warm-up.");
		}
		else if (m_nvperf.GetInitStatus() == nv::perf::profiler::ReportGeneratorInitStatus::Succeeded)
		{
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Hit <space-bar> to begin profiling.");
		}
		else
		{
			title.replace(startNvPerfTitlePos, title.length() - startNvPerfTitlePos, " - Nsight Perf: Initialization failed. Please check the logs.");
		}
#endif
	}
