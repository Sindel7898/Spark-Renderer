...
bool VulkanExampleBase::initVulkan()
{
	...
	std::stable_sort(physicalDevices.begin(), physicalDevices.end(), [](VkPhysicalDevice lhs, VkPhysicalDevice rhs) {
		VkPhysicalDeviceProperties lhsProperties, rhsProperties;
		vkGetPhysicalDeviceProperties(lhs, &lhsProperties);
		vkGetPhysicalDeviceProperties(rhs, &rhsProperties);

		const bool isLhsNvidiaDevice = (lhsProperties.vendorID == 0x10DE);
		const bool isRhsNvidiaDevice = (rhsProperties.vendorID == 0x10DE);
		if (isRhsNvidiaDevice || (lhsProperties.vendorID == rhsProperties.vendorID))
		{
			return false;
		}
		else if (isLhsNvidiaDevice)
		{
			return true;
		}
		else // !isLhsNvidiaDevice && !isRhsNvidiaDevice
		{
			return lhsProperties.vendorID < rhsProperties.vendorID;
		}
	});
#endif
