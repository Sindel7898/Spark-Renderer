...
bool Initialize(ClockControlState& clockControlState)
{
    ...
    {
        if (!VulkanIsNvidiaDevice(clockControlState.physicalDevices[deviceIdx]))
        {
            continue;
        }
...
bool DoStatus(ClockControlState& clockControlState)
{
    for (uint32_t deviceIdx = 0; deviceIdx < clockControlState.numDevices; ++deviceIdx)
    {
        ...
        else
        {
            ...
            ClockInfo clockInfo = VulkanGetDeviceClockState(
                clockControlState.instance,
                clockControlState.physicalDevices[deviceIdx],
                clockControlState.logicalDevices[deviceIdx]);

            printf("[%u] %-17s - %s\n", deviceIdx, deviceIdentifiers.pDeviceName, ToCString(clockInfo));
        }
    }
...
bool DoLockUnlock(ClockControlState& clockControlState)
{
    ...
    if (clockControlState.command == Command::LockToRatedTDP)
    {
        clockSetting = NVPW_DEVICE_CLOCK_SETTING_LOCK_TO_RATED_TDP;
        clockSettingStr = "Locked to rated TDP";
    }
    else if (clockControlState.command == Command::LockToTurboBoost)
    {
        clockSetting = NVPW_DEVICE_CLOCK_SETTING_LOCK_TO_TURBO_BOOST;
        clockSettingStr = "Locked to turbo boost rate";
    }
...
void PrintUsage()
{
    ...
    printf("  lock_to_rated_tdp   - lock the clock to rated tdp per requested device\n");
    printf("  lock_to_turbo_boost - lock the clock to turbo boost per requested device\n");
...
bool ParseArguments(const int argc, const char* argv[], ClockControlState& clockControlState)
{
    ...
    else if (!strcmp(argv[1], "lock_to_rated_tdp"))
    {
        clockControlState.command = Command::LockToRatedTDP;
    }
    else if (!strcmp(argv[1], "lock_to_turbo_boost"))
    {
        clockControlState.command = Command::LockToTurboBoost;
    }
...
int main(const int argc, const char* argv[])
{
    ...
    switch (clockControlState.command)
    {
        case Command::Status:
            ...
            break;
        case Command::LockToRatedTDP:
        case Command::LockToTurboBoost:
        case Command::Unlock:
