...
namespace nv { namespace perf { namespace tool { namespace dx {
    ...
    inline void AppendDeviceState(const State& state, size_t deviceIndex, ordered_json& node)
    {
        ...
        node["ProfilerIsGpuSupported"]["SkuSupportLevel"] = nullptr;
        ...
        [&]() {
            ...
            node["ProfilerIsGpuSupported"]["SkuSupportLevel"] = true;
            if (!isGpuSupportedParams.isSupported)
            {
                ...
                if (isGpuSupportedParams.skuSupportLevel == NVPW_SKU_SUPPORT_LEVEL_UNSUPPORTED)
                {
                    node["ProfilerIsGpuSupported"]["SkuSupportLevel"] = false;
                    unsupportedReason += "GPU SKU is not supported. See https://developer.nvidia.com/err_nvgpu;";
                }
