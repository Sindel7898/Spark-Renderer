...
void main() {
    ...
    if(KD.KernelSizeRadiusBiasAndBool.w == 1){
        ...
        vec3 randomVec = textureLod(samplerNoise, inTexCoord * (TextureSize / 4.0), 0).rgb;
        ...
        for(int i = 0; i < KD.KernelSizeRadiusBiasAndBool.x; ++i){
            ...
            vec3 sampledWorldPos = textureLod(samplerposition, offset.xy, 0).rgb;
            vec4 sampledViewPos = KD.CameraViewMatrix * vec4(sampledWorldPos, 1.0);
            float sampleDepth = sampledViewPos.z;
